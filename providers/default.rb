...
action :install do
  ...
  if version >= '8.1.0'.to_version
    ...
    converge_by("Downloading #{ @new_resource.name }") do
      ...
      install_energyplus(filename, extracted_dir_name, true)
    end
    ...
    converge_by("Downloading #{ @new_resource.name }") do
      download_energyplus(file_path, src_path)
      install_energyplus(filename, extracted_dir_name)
    end
  else
...
def install_energyplus(filename, extracted_dir_name, extract_to_known_dir = false)
  ...
  bash "install_energyplus" do
    ...
    code <<-EOH
      #{tar_command}
      mv #{extracted_dir_name} /usr/local/

      cd /usr/local/bin
      find ../#{extracted_dir_name}/EnergyPlus -type f -perm -o+rx -exec ln -s {} \\;
      find ../#{extracted_dir_name}/ExpandObjects -type f -perm -o+rx -exec ln -s {} \\;
      find ../#{extracted_dir_name}/EPMacro -type f -perm -o+rx -exec ln -s {} \\;

      # in some versions the file is downcased and in the bin
      #find ../#{extracted_dir_name}/bin/energyplus -type f -perm -o+rx -exec ln -s {} \\;
    EOH
